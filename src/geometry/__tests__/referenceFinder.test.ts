...
import {parseReferenceCoordinate,findOrigamiProportionCandidate} from '../referenceFinder';
...
it('identifies rational and quadratic origami proportions correctly',()=>{
 expect(findOrigamiProportionCandidate(0.375)?.label).toBe('3/8');
 expect(findOrigamiProportionCandidate(Math.SQRT2 - 1)?.label).toBe('√2 − 1');
 expect(findOrigamiProportionCandidate((Math.SQRT2 - 1) / 2)?.label).toBe('(√2 − 1) / 2');
 expect(findOrigamiProportionCandidate(1/3)?.label).toBe('1/3');
 expect(findOrigamiProportionCandidate(0.625)?.label).toBe('5/8');
 expect(findOrigamiProportionCandidate(0.25)?.label).toBe('1/4');
});
